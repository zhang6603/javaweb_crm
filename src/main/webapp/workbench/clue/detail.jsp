...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#convertBtn").click(function () {
			window.location.href="workbench/clue/convert.jsp?clueId=${c.id}&fullname=${c.fullname}&appellation=${c.appellation}&company=${c.company}&owner=${c.owner}"
		})
		...
		$("#aname").keydown(function (event) {
			if (event.keyCode==13){
				//显示市场活动列表
				$.ajax({
					url:"workbench/clue/getActivityListByNameAndNotByClue.do",
					data:{
						"clueId":"${c.id}",
						"name":$.trim($("#aname").val())
					},
		...
		$("#BundActivityBody").on("click",$("input[name=xz]"),function () {
			$("#qx").prop("checked",$("input[name=xz]").length==$("input[name=xz]:checked").length?true:false)
		})
